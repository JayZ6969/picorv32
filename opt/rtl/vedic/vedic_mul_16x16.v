...
reg  [PROD_W-1:0] csa_l1_sum_r;
reg  [PROD_W-1:0] csa_l1_carry_r;
reg  [PROD_W-1:0] t_hh_r;
wire [PROD_W-1:0] csa_l2_sum;
wire [PROD_W-1:0] csa_l2_carry;
wire [PROD_W-1:0] p_ksa;
wire              p_ksa_cout_unused;

vedic_mul_8x8 #(.WIDTH(HALF)) u_ll (.A(A[HALF-1:0]),      .B(B[HALF-1:0]),      .P(P_LL));
vedic_mul_8x8 #(.WIDTH(HALF)) u_lh (.A(A[HALF-1:0]),      .B(B[WIDTH-1:HALF]),  .P(P_LH));
vedic_mul_8x8 #(.WIDTH(HALF)) u_hl (.A(A[WIDTH-1:HALF]),  .B(B[HALF-1:0]),      .P(P_HL));
vedic_mul_8x8 #(.WIDTH(HALF)) u_hh (.A(A[WIDTH-1:HALF]),  .B(B[WIDTH-1:HALF]),  .P(P_HH));
...
// Pipeline cut: register CSA L1 outputs and the HH partial.
always @(posedge clk) begin
    csa_l1_sum_r <= csa_l1_sum;
    csa_l1_carry_r <= csa_l1_carry;
    t_hh_r <= t_hh;
end
